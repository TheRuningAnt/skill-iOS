...
@private
    ...
    __block NSInteger _currentTimeStamp;
    __block NSInteger _lastYearIndex;
    __block NSInteger _lastMonthIndex;
    __block NSInteger _lastDayIndex;
...
-(instancetype)createTimePickerWithFrame:(CGRect)frame rowHeight:(CGFloat)rowHeight fontSize:(CGFloat)fontSize fontColor:(UIColor*)fontColor selectAction:(void (^)(UIPickerView *timePickView,NSInteger compoent,NSInteger row,NSString *timeStr,NSInteger timeInterval))action{
    ...
    id res = [self initWithFrame:frame firstsTitles:_years
                     firstAction:^(UIPickerView *pickView, NSInteger compoent, NSInteger row) {
                         ...
                         //根据年份和月份来调整days的数目
                         NSInteger year = [[_years[_lastYearIndex] substringToIndex:4] integerValue];
                         NSInteger month = _lastMonthIndex + 1;
                         ...
                         if ((year%4==0 && year%100!=0) || year%400 ==0){
                             ...
                             if (month == 2) {
                                 ...
                                 NSInteger daysCount = _days.count ;
                         ...
                         if (month == 1 ||month == 3 ||month == 5 ||month == 7 ||month == 8 ||month == 10 ||month == 12) {
                             ...
                             while (_days.count < 31) {
                                 ...
                                 NSInteger dayCount = [[[_days lastObject] substringToIndex:3] integerValue];
 ...
 */
-(void)selectTimeWithTimeStamp:(NSInteger)timeStamp{
    ...
    NSString *monthStr = [NSString stringWithFormat:@"%lu月",[timeTags[1] integerValue]];
    NSString *dayStr = [NSString stringWithFormat:@"%lu日",[timeTags[2] integerValue]];
    
    NSInteger yearIndex = [_years indexOfObject:yearStr];
    NSInteger monthIndex = [_months indexOfObject:monthStr];
    NSInteger dayIndex = [_days indexOfObject:dayStr];
    
    NSInteger year = [timeTags[0] integerValue];
    NSInteger month = [timeTags[1] integerValue];
...
/**
 ...
 @param day 日
 */
-(void)selectTimeWithYear:(NSInteger)year month:(NSInteger)month day:(NSInteger)day{
    
    if (!_years || !_months || !_days || !_pickView)return;
    
    NSInteger yearIndex = [_years indexOfObject:[NSString stringWithFormat:@"%lu年",year]];
    NSInteger monthIndex = [_months indexOfObject:[NSString stringWithFormat:@"%lu月",month]];
    NSInteger dayIndex = [_days indexOfObject:[NSString stringWithFormat:@"%lu日",day]];
...
/**
 ...
 @return 葡萄藤专用地址选择器
 */
-(instancetype)createPTTPlacePickerWithFrame:(CGRect)frame rowHeight:(CGFloat)rowHeight fontSize:(CGFloat)fontSize fontColor:(UIColor*)fontColor selectAction:(void (^)(UIPickerView *pickView,NSString * currentProvince,NSString* proviceNumber,NSString *currentCity,NSString *cityNumber))action{
    ...
    id res = [self initWithFrame:frame
                    firstsTitles:_pttProvinces
                     firstAction:^(UIPickerView *pickView, NSInteger compoent, NSInteger row) {
                         ...
                         //回调方法
                         action(weakSelf.pickView,_pttCurrentProvince,[_pttProvinceRootDic objectForKey:_pttCurrentProvince],_pttCurrentCity,[citiesDic objectForKey:_pttCurrentCity]);
                     }
...
/**
 ...
 @return 两个id对应的城市名
 */
+(NSString*)pttCityOfProvinceId:(NSInteger)provinceId andCityId:(NSInteger)cityId{
...
/**
 ...
 @param cityId 城市id
 */
-(void)selectPlaceWithProvinceId:(NSInteger)provinceId andCityId:(NSInteger)cityId{
    ...
    //获取省份名和城市名对应的下标
    NSInteger provinceIndex = [_pttProvinces indexOfObject:province];
    NSInteger cityIndex = [_pttCurrentCities indexOfObject:cityName];
