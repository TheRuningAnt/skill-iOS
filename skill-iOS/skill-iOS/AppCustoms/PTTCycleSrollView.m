...
/**
 *  使用给定的网络图片数组和描述数组来更新scrollView   目前只考虑到网络图片数据更新问题  本体图片数据更新待检测!!!
 *
    ...
    if (self.localImagesName) {         //若是读取本地图片,从本地加载数据
        if (self.localImagesName.count != 0) {
            [self.localImagesName enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                UIImageView *imageView = [[UIImageView alloc] initWithframe:self.frame andImage:obj contendMode:PTT_ImageView_Image_Contend_Mode_Fill withAction:^(id sender) {
...
-(UIScrollView *)pttScrollView{
    if (!_pttScrollView) {
        ...
        _pttScrollView = [[UIScrollView alloc] initWithFrame:rect];
    }
