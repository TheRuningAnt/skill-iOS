...
-(void)viewWillAppear:(BOOL)animated{
    ...
    //启动友盟页面统计
    [MobClick beginLogPageView:@"首页模块"];
}
...
- (void)viewDidLoad {
    ...
    //如果用户第一次登录 则提示报名流程
    if ([UserTool userModel].firstLogin) {
        
        [self addRegisterTipView];
    }
}
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [MobClick endLogPageView:@"首页模块"];
}

//初始化数据
-(void)initializeData{
    
    [self.headImages addObjectsFromArray:@[@"banner1",@"banner2",@"banner3"]];
}
...
//创建子控件
-(void)addRegisterTipView{
    
    RegisterSuccessView *registSuccessTip = [[RegisterSuccessView alloc] initWithFrame:[UIScreen mainScreen].bounds];
    registSuccessTip.responder = self;
    [[UIApplication sharedApplication].keyWindow addSubview:registSuccessTip];
}

#pragma mark 更多  按钮点击触发
-(void)clickMoreBtn{
...
-(UICollectionView *)jobListCollectionView{
    ...
    layout.itemSize = CGSizeMake(kWindowWidth/2 - 1 , kWindowWidth/2/4*3 + 80);
