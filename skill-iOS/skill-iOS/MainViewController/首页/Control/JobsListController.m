...
-(void)viewWillAppear:(BOOL)animated{
    ...
    //启动友盟页面统计
    [MobClick beginLogPageView:@"职业列表模块"];
}
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [MobClick endLogPageView:@"职业列表模块"];
}
...
-(void)loadMoreData{
    ...
    [ShowMessageTipUtil showTipLabelWithMessage:@"没有更多职业了" spacingWithTop:kWindowHeight/2 stayTime:2];
...
-(UICollectionView *)jobListCollectionView{
    ...
    if (!_jobListCollectionView) {
        ...
        _jobListCollectionView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
            // 进入刷新状态后会自动调用这个block
