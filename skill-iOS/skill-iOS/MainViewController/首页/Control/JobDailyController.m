...
-(void)viewWillAppear:(BOOL)animated{
    ...
    //启动友盟页面统计
    [MobClick beginLogPageView:@"职业日报模块"];
}
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [MobClick endLogPageView:@"职业日报模块"];
...
#pragma mark testCode
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            //如果没有新数据的话,则给用户提示
            if (numberBeforeLoad == weakSelf.jobDailyModels.count && [[paramensDic objectForKey:@"page"] integerValue] != 1) {
                
                [ShowMessageTipUtil showTipLabelWithMessage:@"没有更多日报了" spacingWithTop:kWindowHeight/2 stayTime:2];
            }
            ...
            //如果日报数组里有数据并且没有数据的提示图片已经添加到了视图上,则将其移除掉
            if (weakSelf.jobDailyModels.count > 0 && weakSelf.nonDailyImageV.superview) {
                
                [weakSelf.nonDailyImageV removeFromSuperview];
            }
...
-(UITableView *)jobDailyTableView{
    ...
    if (!_jobDailyTableView) {
        ...
        _jobDailyTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
