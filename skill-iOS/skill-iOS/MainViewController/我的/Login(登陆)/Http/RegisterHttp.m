...
//        manger.responseSerializer = [AFHTTPResponseSerializer serializer];
//    
//    NSString *urlStr = [NSString stringWithFormat:@"%@%@",[API_General stringByAppendingString:,API_SSO_Login];
//    
//    NSString* url = [urlStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
+(void)getMobileNumberIsRegister:(NSString *)openid Type:(NSString *)type successBlock:(void (^)(id))block{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@%@?openid=%@&type=%@",API_General,openid,type];
...
+ (NSString *)checkPassword:(NSString *)aPassword
{
    ...
    NSString *CM_NUM = @"^[a-z0-9]{6,16}$";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", CM_NUM];
    BOOL isMatch = [pred evaluateWithObject:aPassword];

    if (!isMatch) {
        return @"请输入6-16位由字母或数字组成的密码";
    }
    return  nil;
}
