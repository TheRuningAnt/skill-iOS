...
@interface SwitchoverThreeTableView()<UIScrollViewDelegate>
{
    UIButton *firstButton;   //第一个按钮
    UIButton *secondButton;  //第二个按钮
    UIButton *thirdButton;   //第三个按钮
    ...
    UIView *_thirdView;         //指向第三个控件
    
    void (^firstBlock)() ;   //第一个按钮触发Block
    void (^secondBlock)() ;  //第二个按钮出发Block
    void (^thirdBlock)() ;   //第三个按钮出发Block
    
}

@end

@implementation SwitchoverThreeTableView
...
-(instancetype)initWithFrame:(CGRect)frame FirstBtnTitle:(NSString*)firstTitle  firstView:(UIView*)firstView firstAction:(void (^)())firstAction sectionBtnTitle:(NSString*)secondTitle secondView:(UIView*)secondView secondAction:(void (^)())secondAction thirdBtnTitle:(NSString*)thirdTitle thirdView:(UIView*)thirdView thirdAction:(void (^)())thirdAction tabBaeHidden:(BOOL)tabBarHidden{
    ...
    thirdBlock = thirdAction;
    
    //创建第一个Btn tag值为1的时候为选中状态 2为非选择状态
    firstButton = [UIButton buttonWithType:UIButtonTypeCustom];
    firstButton.frame = CGRectMake(0, 0, kWindowWidth/3.f - 2, 45*HEIGHT_SCALE);
    ...
    secondButton.frame = CGRectMake(kWindowWidth/3.f - 2, 0, kWindowWidth/3.f - 0.5, 45*HEIGHT_SCALE);
    ...
    //创建第三个Btn tag值为1的时候为选中状态 2为非选择状态
    thirdButton = [UIButton buttonWithType:UIButtonTypeCustom];
    thirdButton.frame = CGRectMake(kWindowWidth/3.f*2 - 2, 0, kWindowWidth/3.f - 0.5, 45*HEIGHT_SCALE);
    [thirdButton setTitle:thirdTitle forState:UIControlStateNormal];
    [thirdButton setTitleColor:color_7892a5 forState:UIControlStateNormal];
    [thirdButton setTitleColor:color_51d4b9 forState:UIControlStateSelected];
    thirdButton.backgroundColor = [UIColor whiteColor];
    thirdButton.titleLabel.font = [UIFont systemFontOfSize:16*WIDTH_SCALE];
    [thirdButton addTarget:self action:@selector(thirdBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:thirdButton];
    
    //创建中间的第一条修饰条
    UIImageView *tip1 = [[UIImageView alloc] initWithFrame:CGRectMake(kWindowWidth/3.f - 1, 0, 1, 45*HEIGHT_SCALE)];
    tip1.image = [UIImage imageNamed:@"Switch-two-view-tip"];
    [self addSubview:tip1];
    
    //创建中间的第二条修饰条
    UIImageView *tip2 = [[UIImageView alloc] initWithFrame:CGRectMake(kWindowWidth/3.f*2 - 2, 0, 1, 45*HEIGHT_SCALE)];
    tip2.image = [UIImage imageNamed:@"Switch-two-view-tip"];
    [self addSubview:tip2];
    
    //创建下方点击指示图
    changeTipView = [[UIView alloc] initWithFrame:CGRectMake(0, 45*HEIGHT_SCALE - 2, kWindowWidth/3.f, 2)];
    ...
    //创建子视图容器
    contentView = [[UIView alloc] initWithFrame:CGRectMake(0, 45*HEIGHT_SCALE, kWindowWidth*3, self.height - 45*HEIGHT_SCALE)];
    ...
    if(thirdView){
        ...
        if(tabBarHidden){
            ...
            //状态栏隐藏
            thirdView.frame = CGRectMake(kWindowWidth*2, 0, kWindowWidth, contentView.height);
        }else{
            //不隐藏
            thirdView.frame = CGRectMake(kWindowWidth*2, 0, kWindowWidth, contentView.height - 20);
        }
...
-(void)changHeight:(CGFloat)value{
    ...
    _thirdView.frame = [self changeFrame:_thirdView.frame withHeight:value];
}
...
-(void)firstBtnClick:(id)sender{
    ...
    if (changeTipView.origin.x != 0) {
        [UIView animateWithDuration:0.2 animations:^{
            changeTipView.frame = CGRectMake(0, 45*HEIGHT_SCALE - 2, kWindowWidth/3.f, 2);
        }];
    }
...
-(void)secondBtnClick:(id)sender{
    ...
    //改变指示视图的位置
    if (changeTipView.origin.x != kWindowWidth/3.f) {
        [UIView animateWithDuration:0.2 animations:^{
            changeTipView.frame = CGRectMake(kWindowWidth/3.f, 45*HEIGHT_SCALE - 2, kWindowWidth/3.f, 2);
        }];
    }
    
    //更改下方子视图位置
    if (contentView.frame.origin.x != kWindowWidth/3.f) {
...
-(void)thirdBtnClick:(id)sender{
    ...
    //改变指示视图的位置
    if (changeTipView.origin.x != kWindowWidth/3.f*2) {
        [UIView animateWithDuration:0.2 animations:^{
            changeTipView.frame = CGRectMake(kWindowWidth/3.f*2, 45*HEIGHT_SCALE - 2, kWindowWidth/3.f, 2);
        }];
    }
    ...
    if (contentView.frame.origin.x != kWindowWidth) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            contentView.frame = CGRectMake(-kWindowWidth*2.f, 45*HEIGHT_SCALE, contentView.frame.size.width, contentView.frame.size.height);
        }];
    }
